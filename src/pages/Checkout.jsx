...
function normalizeCartItem(item = {}) {
  return {
    ...
    pax: Math.max(1, toNumber(item.pax, 1)),
    price: toNumber(item.price),
    audience: item.audience === "foreign" ? "foreign" : "domestic",
  };
}
...
export default function Checkout() {
  ...
  const normalizedItems = useMemo(() => items.map(normalizeCartItem), [items]);
  const lineSubtotal = useCallback((item) => item.price * item.pax * item.qty, []);

  const grandTotal = useMemo(
    () => normalizedItems.reduce((sum, item) => sum + lineSubtotal(item), 0),
    [normalizedItems, lineSubtotal]
  );

  const first = normalizedItems[0] || null;
  ...
  const hasMixedSelections = useMemo(() => {
    if (!first) return false;
    return normalizedItems.some(
      (item) =>
        item.id !== first.id ||
        item.pax !== first.pax ||
        item.audience !== first.audience
    );
  }, [first, normalizedItems]);

  const labelForAudience = useCallback(
    (value) =>
      value === "foreign"
        ? t("checkout.audienceForeign")
        : t("checkout.audienceDomestic"),
  ...
  const buildWAMessage = useCallback(
    (summary) => {
      ...
      lines.push(
        `${t("checkout.wa.audience")}: ${labelForAudience(summary.audience)}`
      );
      lines.push("");
      lines.push(t("checkout.wa.items"));
      summary.items.forEach((item) => {
        lines.push(
          `- ${item.title} (${labelForAudience(item.audience)}) - ${item.pax} ${t(
            "home.pax"
          )} x ${item.qty} = ${formatMoneyFromIDR(
            lineSubtotal(item),
            currency,
            fx,
            locale
      ...
      lines.push("");
      lines.push(
        `${t("checkout.wa.total")}: ${formatMoneyFromIDR(
          summary.grandTotal,
          currency,
          fx,
          locale
      ...
      if (summary.public_code) {
        lines.push(`${t("checkout.wa.code")}: ${summary.public_code}`);
      }
      lines.push("");
      lines.push(t("checkout.wa.footer"));
      return encodeURIComponent(lines.join("\n"));
    },
    [currency, fx, labelForAudience, lineSubtotal, locale, t]
  );

  const buildWAFromSnapshot = useCallback(
    (snap) => {
      const lines = [];
      lines.push(t("checkout.wa.header"));
      lines.push("");
  ...
  const snapshotFromCurrent = useCallback(
    ...
    [audience, form, grandTotal, i18n.language, normalizedItems]
  );
  ...
  const onSubmit = async (event) => {
    ...
    if (hasMixedSelections) {
      setMsg(t("checkout.singlePackageOnly"));
      return;
    }
    ...
    try {
      ...
      const payload = {
        ...
        p_items: normalizedItems.map((item) => ({
          item_name: `${item.title} (${labelForAudience(item.audience)})`,
          qty: item.qty,
          price_idr: item.price * item.pax,
        })),
      };
            ...
            >
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">
                {t("checkout.ticketInfo", {
                  defaultValue: "Used for e-ticket confirmation.",
                })}
              </p>
              ...
              <form id="checkout-form" onSubmit={onSubmit} className="space-y-5">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative group">
                    <FileText
                      className="absolute left-3.5 top-4 text-slate-400 group-focus-within:text-sky-500 transition-colors"
                      size={18}
                    />
                    <textarea
                      rows="3"
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 md:p-8 shadow-sm border border-slate-100 dark:border-slate-800 opacity-80 pointer-events-none grayscale-[0.5]">
              ...
              <p className="text-sm text-slate-500">
                {t("checkout.secureDesc", {
                  defaultValue: "Managed via WhatsApp securely.",
                })}
              </p>
            </div>
          </div>
              ...
              <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-lg shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-slate-800 overflow-hidden">
                ...
                <div className="p-6 space-y-5">
                  {normalizedItems.length === 0 ? (
                    ...
                    <div className="space-y-4">
                      {normalizedItems.map((item, index) => (
                        <div
                          key={`${item.id || item.title}-${item.pax}-${item.audience}-${index}`}
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-slate-500 mt-0.5">
                              {item.pax} {t("home.pax")} x {item.qty}{" "}
                  ...
                  <button
                    form="checkout-form"
                    disabled={
                      loading || normalizedItems.length === 0 || hasMixedSelections
                    }
                    className="w-full btn btn-primary py-4 rounded-xl font-bold text-base shadow-lg shadow-sky-500/20 hover:shadow-sky-500/30 transition-all flex items-center justify-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed"
                  >
                    {loading ? (
                  ...
                  {(msg || hasMixedSelections) && (
                    <p className="text-center text-sm text-red-500 mt-2 bg-red-50 dark:bg-red-900/20 py-2 rounded-lg">
                      {msg ||
                        t("checkout.singlePackageOnly", {
                          defaultValue:
                            "Please checkout one package selection at a time.",
                        })}
                    </p>
                  )}
            ...
            >
              ...
              <div className="p-6 relative">
                ...
                <div className="space-y-4 text-center">
                  ...
                  <div className="flex items-center justify-between text-sm px-2">
                    ...
                    <span className="font-bold text-slate-900 dark:text-white">
                      {formatMoneyFromIDR(
                        toNumber(justPlaced.totals?.grand),
                        currency,
                        fx,
                        locale
                      )}
                ...
                <div className="mt-8 space-y-3">
                  <a
                    href={`https://wa.me/${WA_NUMBER}?text=${buildWAFromSnapshot(
                      justPlaced
                    )}`}
                    target="_blank"
                    rel="noreferrer"
